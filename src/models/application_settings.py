...
class ApplicationSettings:
    ...
    Attributes:
        ...
        workflow_configs: List of 13 workflow configurations (name + path pairs)
    ...
    workflow_configs: list[WorkflowConfig] = field(
        default_factory=lambda: [WorkflowConfig(name="", path="") for _ in range(13)]
    )
    ...
    def __post_init__(self) -> None:
        ...
        # Ensure workflow_configs has exactly 13 entries
        if len(self.workflow_configs) < 13:
            while len(self.workflow_configs) < 13:
    ...
    def from_dict(cls, data: dict) -> "ApplicationSettings":
        ...
        # Pad to exactly 13 slots (handles older settings files with fewer entries)
        while len(workflow_configs_data) < 13:
