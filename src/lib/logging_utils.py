...
class LoggerManager:
    ...
    def setup_file_logging(
        cls, log_dir: str = "logs", log_level: int = logging.DEBUG
    ) -> None:
...
def log_error_with_context(
    error: Exception, operation: str, context: Optional[dict] = None
) -> None:
...
def log_retry_attempt(operation: str, attempt: int, max_attempts: int) -> None:
    ...
    """
    logger = LoggerManager.get_logger()
    logger.warning(f"Retry attempt {attempt}/{max_attempts} for {operation}")
