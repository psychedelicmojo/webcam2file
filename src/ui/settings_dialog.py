...
class SettingsDialog:
    ...
    def __init__(
        ...
        self._dialog.geometry("700x1700")
    ...
    def _build_ui(self) -> None:
        ...
        self._apps_script_url_entry.grid(row=29, column=1, sticky="ew", pady=5, padx=5)
        ttk.Label(
            main_frame,
            text="Google Apps Script web app URL for email sending",
            foreground="gray",
        ).grid(row=29, column=2, sticky="w", padx=5)

        # API timeout section
        ttk.Label(main_frame, text="API Timeout (seconds):").grid(
            row=30, column=0, sticky="w", pady=5
        )
        ...
        self._api_timeout_entry.grid(row=30, column=1, sticky="w", pady=5, padx=5)

        # Countdown seconds section
        ttk.Label(main_frame, text="Capture Countdown (seconds):").grid(
            row=31, column=0, sticky="w", pady=5
        )
        ...
        self._countdown_entry.grid(row=31, column=1, sticky="w", pady=5, padx=5)
        ttk.Label(
            main_frame, text="0 = capture immediately", foreground="gray"
        ).grid(row=31, column=2, sticky="w", padx=5)

        # Status label
        self._status_label = ttk.Label(main_frame, text="", foreground="blue")
        self._status_label.grid(row=32, column=0, columnspan=3, pady=10)

        # Buttons frame
        buttons_frame = ttk.Frame(main_frame)
        buttons_frame.grid(row=33, column=0, columnspan=3, pady=10)
